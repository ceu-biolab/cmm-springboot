...
public class GCMSSearchService {
    ...
    public GCMSSearchResponseDTO search(GCMSSearchRequestDTO request) {
        ...
        if (RITolerance >= 0 && RITolerance <= 1){//
            RIToleranceFinal = RITolerance;
        } else {
            throw new IllegalArgumentException("Retentention Index Tolerance must be positive" +
                    " and with a value between 0 and 1 (decimal value of the percentage): " + request.getRetentionIndexTolerance());
        }
        ...
        try {
            //TODO CAMBIAR EN FUNCION DE LAS NUEAS QUERYS
            ...
            for (GCMSQueryResponseDTO queryResult : queryResults) {
                ...
                GCMSAnnotation gcmsAnnotation = GCMSAnnotation.builder().gcmsCompound(gcmsCompound)
                        ...
                        .build();
                gcmsAnnotation.cosineScoreFunction();
